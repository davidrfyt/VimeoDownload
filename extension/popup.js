document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Auto-detect Vimeo URL from current tab
    try {
        const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
        if (tab && tab.url && tab.url.includes('vimeo.com')) {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(data.error || 'Ocurrió un error al procesar el video. Asegúrate de que el servidor local esté corriendo.');
            }
    ...
    async function handleDownload(btn, format) {
        ...
        let downloadUrl = `${SERVER_URL}/api/download?url=${encodeURIComponent(url)}&title=${encodeURIComponent(title)}&format=${format}`;
        if (password) {
        ...
        try {
            ...
            alert('Hubo un problema al descargar el archivo. ¿Está el servidor activo?');
        } finally {
